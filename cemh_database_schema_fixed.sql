...
CREATE TABLE `meeting` (
  ...
  `meeting_type` tinyint DEFAULT '1' COMMENT '会议类型：1=线下会议，2=线上会议，3=混合会议',
  `location` varchar(200) DEFAULT NULL COMMENT '会议地点',
  `online_url` varchar(500) DEFAULT NULL COMMENT '线上会议链接',
  `start_time` datetime NOT NULL COMMENT '开始时间',
  `end_time` datetime NOT NULL COMMENT '结束时间',
  `max_participants` int DEFAULT '0' COMMENT '最大参与人数，0表示不限制',
  `current_participants` int DEFAULT '0' COMMENT '当前报名人数',
  `status` tinyint NOT NULL DEFAULT '1' COMMENT '状态：0=草稿，1=已发布，2=进行中，3=已结束，4=已取消',
